    ...
    <body>
        ...
        <form ref="moimForm" @submit.prevent="onSubmitForm">
            ...
            <div class="title-input">
                <input v-model="moimForm.moim_title" class="moim-input-title" placeholder="모임 이름을 입력하세요">
                <div v-if="!moimForm.moim_title && isFormSubmitted" class="invalid-message">모임 이름을 입력해주세요.</div>
            </div>
            ...
            <div class="box">
                ...
                <div class="left">
                    ...
                    <div class="moim-img">
                        <div class="img_box">
                            <label for="input-file" v-if="!moim_img">
                                <h3 class="img-font">+ 모임 메인 사진 추가하기</h3>
                            </label>
                            <label for="input-file" v-if="moim_img">
                    ...
                    <div v-if="!moimForm.moim_img && isFormSubmitted" class="invalid-message">모임 메인 이미지를 선택해주세요.</div>
                    <hr class="AllPosts-hr">

                    <div class="moim-introduce">
                        <h3 class="sub-font">📜 모임을 소개해주세요</h3>
                        <div>
                            <input class="moim-input" v-model="moimForm.moim_intro" placeholder="모임 소개말을 입력하세요(10자이내)">
                            <div v-if="!moimForm.moim_intro && isFormSubmitted" class="invalid-message">모임 소개말을 입력해주세요.
                            </div>
                        </div>
                        <textarea v-model="moimForm.moim_con" placeholder="모임 소개를 자세히 입력해주세요"></textarea>
                        <div v-if="!moimForm.moim_con && isFormSubmitted" class="invalid-message">모임 소개를 입력해주세요.</div>
                    </div>

                    <hr class="AllPosts-hr">
                    <h3 class="sub-font moim-cate">🏷️ 모임 분류</h3>
                    <div class="cate-nav">
                        <ul class="cate">
                            ...
                            <li class="col-xs-1" :class="{ 'active': active === 'travel' }"
                                @click="setActive('travel'), getCateList(400)">✈ 여행</li>
                        ...
                        <div v-if="!activeCategory && isFormSubmitted" class="invalid-message">모임 분류를 선택해주세요.</div>
                    </div>

                    <hr class="AllPosts-hr">
                    <div class="moim-question">
                        <h3 class="sub-font" style=" padding: 0 0 20px 0;">❔ 모임가입 질문을 적어주세요</h3>
                        <div v-for="index in questionCount" :key="index">
                            <div class="input-container">
                                <input class="moim-input-question" v-model="moimForm.mq[index]"
                                    :placeholder="'모임가입 질문을 입력하세요 ' + index">
                    ...
                    <hr class="AllPosts-hr">
                    <div class="moim-max">
                        <h3 class="sub-font">🥂 모임 최대 인원을 선택해주세요</h3>
                        <div>
                            <input class="moim-input" v-model="moimForm.moim_max" placeholder="모임 최대 인원을 입력하세요 (최대 100명)"
                                @input="handleMaxInput">
                            <div v-if="isInvalidInput" class="invalid-message">숫자만 입력해주세요</div>
                            <div v-if="!moimForm.moim_max && isFormSubmitted" class="invalid-message">모임 최대 인원을 입력해주세요.
                            </div>
                        </div>
                    </div>
                    <hr class="AllPosts-hr">
                    <div class="moim-place">
                        <h3 class="sub-font moim-cate">🗺️ 모임 활동 지역을 선택해주세요</h3>
                        <div class="cate-nav">
                            <ul class="cate">
                        ...
                        <div v-if="!moimForm.moim_adr && isFormSubmitted" class="invalid-message">모임 활동 지역을 선택해주세요.</div>
                    </div>
                </div>
                ...
                <div class="right">
                    ...
                    <div class="creatediv">
                        <button type="submit" class="col-xs-1 createbtn">모임 만들기</button>
                    </div>
                </div>
            </div>
        </form>
    ...
    methods: {
        ...
        validationCheck() {
            ...
            if (!this.moimForm.moim_title) {
                emptyFields.push('모임 이름');
            }
            if (!this.moimForm.moim_intro) {
                emptyFields.push('모임 소개말');
            }
            if (!this.moimForm.moim_con) {
                emptyFields.push('모임 소개');
            }
            if (!this.activeCategory) {
                emptyFields.push('모임 카테');
            }
            if (!this.moimForm.moim_max) {
                emptyFields.push('모임 최대 인원');
            }
            if (!this.moimForm.moim_img) {
                emptyFields.push('모임 메인 이미지');
            }
